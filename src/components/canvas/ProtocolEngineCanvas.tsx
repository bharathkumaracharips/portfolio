...
export function ProtocolEngineCanvas({ scrollProgress }: ProtocolEngineCanvasProps) {
  ...
  useEffect(() => {
    ...
    const silverFillLight = new THREE.DirectionalLight(0xffffff, 1.8);
    silverFillLight.position.set(-14, 14, 16);
    scene.add(silverFillLight);

    const warmFillLight = new THREE.DirectionalLight(0xffe6c2, 1.3);
    warmFillLight.position.set(-15, 12, 10);
    scene.add(warmFillLight);

    const cyanFillLight = new THREE.DirectionalLight(0x61e7ff, 0.9);
    ...
    // Master World Group (scaled down comfortably to fit the portfolio sidebar/viewport)
    ...
    const sandPalette = [
      new THREE.Color(0xe5a65d),
      new THREE.Color(0xdf9843),
      new THREE.Color(0xf3ca88),
      new THREE.Color(0xd98e32),
      new THREE.Color(0xedba74),
      new THREE.Color(0xcca878),
      new THREE.Color(0xf5d69d),
      new THREE.Color(0xde9b47),
    ];
    ...
    // =========================================================================
    // GSAP TIMELINE & INTERPOLATION MASTER
    // =========================================================================
    ...
    // Scroll Handler (if scrollProgress not passed via prop)
    const handleScroll = () => {
    ...
    function render() {
      ...
      // Update GSAP Master Timeline from scroll
      masterTimeline.progress(scrollProgressRef.current);

      camera.position.set(
        animationState.camX + mouseX * 0.5,
        animationState.camY - mouseY * 0.5,
        animationState.camZ
      );
